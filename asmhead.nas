...
; PIC关闭一切中断
;	根据AT兼容机的规格，如果要初始化PIC，必须在CLI之前进行，否则有时会挂起，然后在进行PIC初始化


		MOV		AL,0xff
		OUT		0x21,AL
		NOP						; 如果连续执行out命令，有些机种无法正常工作
		OUT		0xa1,AL
...
[INSTRSET "i486p"]				; 想要使用486语言的叙述

		LGDT	[GDTR0]			; 设定临时 GDT
		MOV		EAX,CR0
		AND		EAX,0x7fffffff	; bit31为0，为了禁止頒
		OR		EAX,0x00000001	; bit0为1
		MOV		CR0,EAX
		JMP		pipelineflush
pipelineflush:
...
; 所剩下的
		...
		MOV		ECX,0
		MOV		CL,BYTE [CYLS]
		IMUL	ECX,512*18*2/4	; 从注面数变为字节数/4
		SUB		ECX,512/4		; 减去IPL
		CALL	memcpy
